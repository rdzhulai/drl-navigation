...
finish_positions = [[-0.94, -0.65], [1.17, 1.21], [0.9, -0.74], [-0.4, 0.69]]
...
if (robot.step(timestep) != -1 and initializedEnv != 1):
    ...
    for i_episode in range(num_episodes):
        ...
        env.finish_coords = finish_positions[i_episode % 4]
        state = env.reset()
    ...
    torch.save(policy_net, 'policy0.pth')    
    torch.save(target_net, 'target0.pth')        